...
uint256 hashGenesisBlock("0x330185a21bffe14147b42e732b9312a7834af967339e1a86e0678e7a42885264");
...
{
    int64 nSubsidy = 20 * COIN; // Mining rewards

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 336000); // davcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // davcoin: 3.5 days
static const int64 nTargetSpacing = 4.0 * 60; // davcoin: 2.5 minutes
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x65f920278d55a7f05f62e70a3820e23e19a4595241e9d84457687cdda41b5e15");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Madrid Time 29-05-2021, Chelsea FC wins its second Champions League";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0436774797397adf3605a04564f79b251254db973cbe79e295de74e1c9e64da5b327539f9cbd185de68631512572c09aef22395ad14662bb145c284753ae10f9a2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622482969;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085070357;

        if (fTestNet)
        {
            block.nTime    = 1622482851;
            block.nNonce   = 385320634;
        }
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str()); 
        ...
        assert(block.hashMerkleRoot == uint256("0x42f9cdf4e4ccd2b782712691dfc755f6b14bd749864f9e3ff74d5d8861baf69c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb3, 0xd9 }; // davcoin: increase each by adding 2 to bitcoin's value.
